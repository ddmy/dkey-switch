...
### Canonical Intent -> Command 路由表

| 意图（Canonical Intent） | 首选命令 | 备注 |
|--------------------------|----------|------|
| 切换到指定窗口（标题已知）| `activate-window <关键词> 1 --json` | 先用 find-window 确认 |
| 切换到指定窗口（不确定）  | `find-window <关键词> 5 --json` | 列出候选再让用户确认 |
| 按进程名激活              | `activate-process <进程名> 1 --json` | 适合标题变化的应用 |
| 精确句柄激活              | `activate-handle <句柄> --json` | 句柄来自 list-windows |
| 列出所有窗口              | `list-windows --json` | 调试或让用户选择时使用 |
| 标签切换                  | `Dctrl -1` | 降级场景或应用内标签 |
| 窗口切换（降级）          | `Dalt -1` | 激活失败时的最终降级 |
